...
const updateVersion = (beta = false) => {
	const packageJson = getPackageJson();
	packageJson.version = increaseVersion(packageJson.version, beta);
	fs.writeFileSync(packageJsonPath, `${JSON.stringify(packageJson, null, 2)}\n`, 'utf8');

	const manifest = readManifest(manifestPath);
	manifest.version = increaseVersion(manifest.version, beta);
...
function main(environ) {
	...
	if (configName === "updateBetaVersion") {
		updateVersion(true);
		return [];
	}
